...
//   
//
// Created by Quinton skilling edited and amended by Bolaji Eniwaye
// This file simulates the model and generates spiketimes for the study, paramters may need to be change between simulations
...
double Equations(neuron n, double solns[], int eqn ,double tau_gks)
{
	...
	case 3:
	{
		...
		//val = (s_inf - solns[3]) / tau_gks;
		val = (s_inf - solns[3]) / 75.;
		//val = (s_inf - solns[3]) / 200.0;
		break;
	}
	}
...
int main(int argc, char *argv[])
{
	...
	const int simulationTime = 4e4, transientTime =4e4, learningTime = 2e4;
	...
	double EE = 0.0, EI = 0.00046, II = 0.0013, IE = 5e-4;//IE= 5e-4;

	double EE_vec[1]={1e-6};
	double connectivityVals[4] = { EE, EI, II, IE };
	int hetSize = 200;
	double hetStrength = 20.;
	double hetData[2] = { (double)hetSize, hetStrength };
	double learningRate = 1e-5;
	double learningRate_vec[5] = {1e-5, 5e-5, 1e-4, 1e-3, 1e-2};
	double noiseVal =2e-5;// 3e-5;//5e-5;//0., noiseVec[8] = { 1e-5, 2e-5,5e-5,7e-5,1e-4,3e-5,4e-5,5e-6};
	double gks_vec[8] = {0.0 ,0.3, 0.9, 1.5, 0.15, 0.5, 0.7,1.2};
	//int hetSize_vec[6] = { 10, 50, 100, 200, 250, 400 };
	int hetSize_vec[2] = {250, 400};
	//double dcVals[3] = { 1.22, -0.085};
	//double dcVals[3] = { 1.2, -0.085};
	double dcVals[3] = {1.2, -0.085};
	// dcVals[0] = dcVals[0]-dcVals[0]*.1;
	// dcVals[1] = dcVals[1]-dcVals[1]*.1;
	//double dc_change[15]={ 0.8,0.05,0.065,0.08,0.095,0.11,0.2,0.3,0.4,0.05,0.07,0.9,1,1.2,1.5};
	//double dc_change[16]={ 0.8,0.075,0.085,0.115,135, 0.140,0.155,0.2,0.3,0.4,0.05,0.07,0.9,1,1.2,1.5};
	//double dc_change[16]={ 0.8,0.075,0.085,0.115,135, 0.140,0.155,0.2,0.3,0.4,0.05,0.07,0.9,1,1.2,1.5};
	double dc_change[17]={ 0.8,0.075,0.085,0.1,0.12,0.135, 0.140,0.155,0.2,0.3,0.4,0.05,0.07,0.9,1,1.2,1.5};

	double dc_new[7]={-0.085,-0.24, -0.50, -1.0,-0.1, -0.14, -0.18} ;
	double inh_drop_num[1]={0.0} ;
	double tau_gks_vec[5]={ 100.0 , 115.0 , 130.0 , 150.0 , 170.0 };
	...
	//:/mnt/c/Users/eniwbola/Desktop/Research/Quinton_Proj/noiseSweep/noiseSweep/new_het_scan_stability_4_4_2021/gks_scan_1e2/code$
	...
	for (int ns_ind = 1; ns_ind < 5; ns_ind++)
	{
		
		//dcVals[0] = dcVals[0]-dcVals[0]* dc_change[ns_ind];
		//dcVals[1] = -0.085- 0.085* dc_change[ns_ind];

		dcVals[0] = 1.4-1.4*dc_change[ns_ind];
		//dcVals[1] = dcVals[1]-dcVals[1]* dc_change[ns_ind];
		...
		*/
		...
		{
			hetSize=200; //hetSize_vec[ns_ind];
			nrn[n_ind].ID = n_ind;
			...
			{
				nrn[n_ind].gKs =1.5;// gks_vec[ns_ind];//0.1 for Type 1, 0.8 for Type 2	
			}
			else
			{
				nrn[n_ind].gKs = 1.5; // gks_vec[ns_ind];//0.;//0.1 for Type 1, 0.8 for Type 2
			}
			...
			if (nrn[n_ind].isInhibitory)
			{
				...
				//nrn[n_ind].I_DC = dc_new[ns_ind] * (1 - 0.1*probability(mersenne));// (dcVals[1] + 0.1) - 0.2*probability(mersenne);
					if(   probability(mersenne)<( (inh_drop_num[0]/300.0) )  ){
						nrn[n_ind].I_DC = -.24;// (dcVals[1] + 0.1) - 0.2*probability(mersenne);
					}
		...
		for (int n_ind = 0; n_ind < networkSize; n_ind++)
		{
			...
			for (int conn_ind = 0; conn_ind < conn.connections[n_ind].size(); conn_ind++)
			{
				...
				{//IE
					nrn[n_ind].connWeights[conn_ind] = IE;//IE;
				}
				...
				{//EE
					nrn[n_ind].connWeights[conn_ind] = EE; //EE_vec[ns_ind];

					if (nrn[n_ind].isInHet)
					{
						nrn[n_ind].connWeights[conn_ind] = EE; // EE_vec[ns_ind]*hetStrength;
					}
				}
		...
		for (int t_ind = 0; t_ind <= transientTime + learningTime + 2 * simulationTime; t_ind++)
		{
			...
			else if (t_ind == simulationTime + transientTime + learningTime)
			{
				...
				for (int n_ind = 0; n_ind < networkSize; n_ind++)
				{
					...
					nrn[n_ind].gKs =  1.5;//gks_vec[ns_ind];//0.;
				}
			}
			...
			for (int n_ind = 0; n_ind < networkSize; n_ind++)
			{
				...
				double k1_0 = Equations(nrn[n_ind], solutions, 0, tau_gks_vec[ns_ind]);
				double k1_1 = Equations(nrn[n_ind], solutions, 1, tau_gks_vec[ns_ind]);
				double k1_2 = Equations(nrn[n_ind], solutions, 2, tau_gks_vec[ns_ind]);
				double k1_3 = Equations(nrn[n_ind], solutions, 3, tau_gks_vec[ns_ind]);
				...
				double k2_0 = Equations(nrn[n_ind], solutions, 0, tau_gks_vec[ns_ind]);
				double k2_1 = Equations(nrn[n_ind], solutions, 1, tau_gks_vec[ns_ind]);
				double k2_2 = Equations(nrn[n_ind], solutions, 2, tau_gks_vec[ns_ind]);
				double k2_3 = Equations(nrn[n_ind], solutions, 3, tau_gks_vec[ns_ind]);
				...
				double k3_0 = Equations(nrn[n_ind], solutions, 0 ,tau_gks_vec[ns_ind]);
				double k3_1 = Equations(nrn[n_ind], solutions, 1 ,tau_gks_vec[ns_ind]);
				double k3_2 = Equations(nrn[n_ind], solutions, 2 ,tau_gks_vec[ns_ind]);
				double k3_3 = Equations(nrn[n_ind], solutions, 3 ,tau_gks_vec[ns_ind]);
				...
				double k4_0 = Equations(nrn[n_ind], solutions, 0 ,tau_gks_vec[ns_ind]);
				double k4_1 = Equations(nrn[n_ind], solutions, 1 ,tau_gks_vec[ns_ind]);
				double k4_2 = Equations(nrn[n_ind], solutions, 2 ,tau_gks_vec[ns_ind]);
				double k4_3 = Equations(nrn[n_ind], solutions, 3 ,tau_gks_vec[ns_ind]);
				...
				if (nrn[n_ind].V_n <= -20. && solutions[0] > -20.)
				{
					...
					spike_output << nID << "\t" << hetID << "\t" << t_ind*step<< "\t"<< nrn[n_ind].I_DC << endl;// "\t" << nrn[n_ind].I_DC << "\t" << nrn[n_ind].I_syn_exc << "\t" << nrn[n_ind].I_syn_inhb << endl;
				}
